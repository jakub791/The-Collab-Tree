addLayer("cheese", {
  ...
  laeceaGain() {
    return !hasUpgrade("cheese", "b2")
      ...
      : player.cheese.points.max(0).add(1).log(10).div(20).pow(1.69)
          ...
          .mul(Decimal.pow(tmp.cheese.laeceaEffect, player.cheese.buyables["ab12"].add(player.cheese.buyables["ab13"])))
          .mul(Decimal.mul(0.05, hasAchievement("cheese", 23)
            ...
            : 0).add(1))
          .pow(Decimal.pow(1.23456789, player.cheese.buyables["ab13"]));
  },
  laeceaEffect() {
  ...
  buyables: {
    ...
    ab12: {
      ...
      display() {
        return `<h3>
          Boosts special point gain by it's own effect and gives 1 free "G" per purchase
          <br>Amount: ${
    ...
    ab13: {
      ...
      display() {
        return `<h3>
          Boosts special points gain by +^1.23456789 and gives 1 free "G" and "O" per purchase
          <br>Amount: ${
            formatWhole(player.cheese.buyables["ab13"])
          }
          Cost: ${
      ...
      cost() {
        return Decimal.pow(5760, player.cheese.buyables["ab13"].add(1).pow(1.45));
      },
      canAfford() {
        return (
          player.cheese.laeceaPoints.gte(this.cost())
