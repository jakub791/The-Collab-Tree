...
function setupLayer(layer) {
    ...
    if (layers[layer].challenges) {
        ...
        for (thing in layers[layer].challenges) {
            if (isPlainObject(layers[layer].challenges[thing])) {
                ...
                if (
                    layers[layer].challenges[thing].completionLimit ===
                    undefined
                )
                    layers[layer].challenges[thing].completionLimit = 1;
                else if (layers[layer].challenges[thing].marked === undefined)
                    layers[layer].challenges[thing].marked = function () {
                        return maxedChallenge(this.layer, this.id);
                    };
            }
        }
    }
    if (layers[layer].buyables) {
        ...
        for (thing in layers[layer].buyables) {
            if (isPlainObject(layers[layer].buyables[thing])) {
                ...
                layers[layer].buyables[thing].canBuy = function () {
                    return canBuyBuyable(this.layer, this.id);
                };
                if (layers[layer].buyables[thing].purchaseLimit === undefined)
                    layers[layer].buyables[thing].purchaseLimit = new Decimal(
                        Infinity
                    );
            }
        }
    ...
    if (layers[layer].startData) {
        ...
        if (data.best !== undefined && data.showBest === undefined)
            layers[layer].showBest = true;
        if (data.total !== undefined && data.showTotal === undefined)
    ...
    if (layers[layer].gainMult === undefined)
        layers[layer].gainMult = decimalOne;
    if (layers[layer].gainExp === undefined) layers[layer].gainExp = decimalOne;
    if (layers[layer].directMult === undefined)
        layers[layer].directMult = decimalOne;
    if (layers[layer].type === undefined) layers[layer].type = "none";
    if (layers[layer].base === undefined || layers[layer].base <= 1)
        layers[layer].base = 2;
    if (layers[layer].softcap === undefined)
        layers[layer].softcap = new Decimal("e1e7");
    if (layers[layer].softcapPower === undefined)
        layers[layer].softcapPower = new Decimal("0.5");
    if (layers[layer].displayRow === undefined)
        layers[layer].displayRow = layers[layer].row;
    if (layers[layer].name === undefined) layers[layer].name = layer;
    if (layers[layer].layerShown === undefined) layers[layer].layerShown = true;
    if (layers[layer].glowColor === undefined)
        layers[layer].glowColor = defaultGlow;
    ...
    if (!TREE_LAYERS[displayRow] && !isNaN(displayRow))
        TREE_LAYERS[displayRow] = [];
    if (!OTHER_LAYERS[displayRow] && isNaN(displayRow))
        OTHER_LAYERS[displayRow] = [];

    ROW_LAYERS[row][layer] = layer;
    let position =
        layers[layer].position !== undefined ? layers[layer].position : layer;

    if (!isNaN(displayRow) || displayRow < 0)
        TREE_LAYERS[displayRow].push({ layer: layer, position: position });
...
function someLayerUnlocked(row) {
    for (layer in ROW_LAYERS[row]) if (player[layer].unlocked) return true;
    return false;
}
...
addLayer("changelog-tab", {
    tabFormat() {
        return [["raw-html", modInfo.changelog]];
    },
    row: "otherside"
});
