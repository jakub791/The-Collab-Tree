...
function powExp3(n, exp) {
	...
	return Decimal.pow(
		10,
		Decimal.pow(10, Decimal.pow(10, n.log10().log10().log10().pow(exp))),
	);
}

function powExpN(num, n, exp) {
	// n-late
	num = new Decimal(num);
	exp = new Decimal(exp);
...
function getLogisticTimeConstant(current, gain, loss) {
	...
	if (current.gt(gain.div(loss)))
		return current.times(loss).sub(gain).ln().div(-1).div(loss);
