...
function standard(decimal, precision) {
  ...
  if (
    (decimal.layer > 4 && decimal.mag >= 0) ||
    (decimal.mag >= Math.log10(3e123) && decimal.layer == 4)
  ) {
...
function formatSciEng(decimal, precision) {
  ...
  if (
    ...
    if (decimal.layer < 1 || (Math.abs(decimal.mag) < 63 && decimal.layer == 1))
      return standard(decimal, precision);
  }
...
function formatTimeLong(s) {
  ...
  let scale1 = [
    5.39121e-44, 1e-30, 1e-27, 1e-24, 1e-21, 1e-18, 1e-15, 1e-12, 1e-9, 1e-6,
    0.001, 1, 60, 3600, 86400, 31556952, 31556952e3, 31556952e9, 31556952e40,
    31556952e100,
  ];
  let scale2 = [
    " Planck Times",
...
function formatSize(s) {
  ...
  let scale1 = [
    1.616255e-35, 1e-30, 1e-27, 1e-24, 1e-21, 1e-18, 1e-15, 1e-12, 1e-9, 1e-6,
    0.001, 0.01, 1, 1e3, 1e6, 1e9, 1.495978707e11, 9.46e15, 8.8e26,
  ];
  let scale2 = [
    " Planck Lengths",
    " quectometers",
...
function eventsTime(years) {
  ...
  let dates = [
    5388e5, 2521e5, 2013e5, 145e6, 66e6, 5.332e6, 3.5e6, 2.58e6, 7.81e5, 3.15e5,
    2.5e5, 1.95e5, 1.6e5, 1.25e5, 7e4, 6.7e4, 5e4, 4.5e4, 4e4, 3.5e4, 3.3e4,
    3.1e4, 2.9e4, 2.8e4, 2e4, 1.6e4, 1.5e4, 1.4e4, 11600, 1e4, 8e3, 6e3, 5e3,
    4e3, 3200, 3000, 2600, 2500, 2300, 1800, 1400, 1175, 800, 753, 653, 539,
    356, 200, 4, 0,
  ];
  let events = [
    "Cambrian Period",
